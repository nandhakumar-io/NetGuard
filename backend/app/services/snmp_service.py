...
def get_ssh_private_key(device: Device) -> tuple[str, str | None]:
    """Resolve the SSH private key (and optional passphrase) for a device
    configured with ssh_auth_method="key". Returns (private_key_pem,
    passphrase_or_none). Raises CredentialNotFoundError if no key is on
    file -- callers should not silently fall back to password auth here,
    since that would defeat the point of an operator explicitly choosing
    key-based auth for a device.
    """
    if not device.ssh_private_key_encrypted:
        raise CredentialNotFoundError(
            f"Device '{device.hostname}' is set to ssh_auth_method=key but has no private key "
            "configured. Set one via POST /devices/{id}/ssh-credentials."
        )
    key = crypto.decrypt(device.ssh_private_key_encrypted)
    if not key:
        raise CredentialNotFoundError(
            f"Device '{device.hostname}' has a stored SSH private key that could not be decrypted "
            "(corrupt data, or SECRET_ENCRYPTION_KEY changed since it was saved)."
        )
    passphrase = None
    if device.ssh_private_key_passphrase_encrypted:
        passphrase = crypto.decrypt(device.ssh_private_key_passphrase_encrypted)
    return key, passphrase


def set_ssh_private_key(device: Device, private_key: str, passphrase: str | None = None) -> None:
    """Encrypts and stores an SSH private key (and optional passphrase)
    directly on the device row. Pass private_key="" to explicitly clear
    both the key and passphrase (falls back to password auth if
    ssh_auth_method is subsequently set back to "password"). Caller is
    responsible for db.commit().
    """
    device.ssh_private_key_encrypted = crypto.encrypt(private_key) if private_key else None
    if not private_key:
        device.ssh_private_key_passphrase_encrypted = None
    elif passphrase is not None:
        device.ssh_private_key_passphrase_encrypted = crypto.encrypt(passphrase) if passphrase else None
