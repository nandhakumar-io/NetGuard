...
from app.services import change_request_digest, compliance_report
...
@router.get("/change-request-digest")
def get_change_request_digest(
    days: int = Query(7, ge=1, le=365, description="Reporting window in days"),
    db: Session = Depends(get_db),
    _=Depends(get_current_user),
):
    """On-demand version of the scheduled weekly digest (see
    app.services.change_request_digest / app.tasks.
    run_weekly_change_request_digest_task) -- same builder, so this
    doesn't wait for Monday's beat run and always reflects live data.
    CSV-only: unlike the compliance report, there's no PDF variant here,
    so `format` isn't a parameter.

    Available to any authenticated user, same rationale as GET
    /reports/compliance -- read-only, no device/config content included.
    """
    digest = change_request_digest.build_digest(db, window_days=days)
    body = change_request_digest.render_csv(digest)
