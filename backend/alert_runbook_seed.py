...
from app.services.runbook_execution_service import INTENT_COMMAND_PREFIX
...
_DEFAULT_RUNBOOKS = [
    {
        ...
        "notes": (
            ...
            "next traffic. Vendor-agnostic: the command actually pushed "
            "is rendered for each device's own vendor at execution time "
            "(Cisco/Arista/Juniper/Linux) rather than a single fixed "
            "string -- see app.services.config_intent_service."
        ),
        "remediation_enabled": True,
        "remediation_action_type": RemediationActionType.RESTART_SERVICE,
        "remediation_label": "Clear ARP cache",
        "remediation_command": f"{INTENT_COMMAND_PREFIX}clear_arp_cache",
    ...
    },
    {
        ...
        "notes": (
            ...
            "Vendor-agnostic like the ARP-clear runbook above; not "
            "supported against Linux devices (no generic CAM-table "
            "equivalent to clear without a specific bridge interface "
            "name), so it's simply not offered as runnable there."
        ),
        "remediation_enabled": True,
        "remediation_action_type": RemediationActionType.RESTART_SERVICE,
        "remediation_label": "Clear MAC address table",
        "remediation_command": f"{INTENT_COMMAND_PREFIX}clear_mac_table",
        "remediation_required_role": None,
    },
    {
