 ...
 * Uint8Array, but the backend hands it over base64url-encoded (the usual
 * transport-safe form) -- this is the standard conversion every Web Push
 * how-to includes for exactly that reason. */
...
export default function PushSettings() {
  ...
  // Browser push (Web Push/VAPID) support -- see /sw.js and
  // urlBase64ToUint8Array above. `vapidPublicKey === null` means the
  // server hasn't got a keypair configured (settings.VAPID_*), in which
  // case the Browser option is hidden entirely rather than offered and
  // failing later.
  const [vapidPublicKey, setVapidPublicKey] = useState<string | null>(null);
  const browserPushSupported =
    typeof window !== "undefined" && "serviceWorker" in navigator && "PushManager" in window;
  const [subscribingBrowser, setSubscribingBrowser] = useState(false);
  ...
  useEffect(() => {
    ...
    if (browserPushSupported) {
      api
        .get<{ configured: boolean; public_key: string | null }>("/push-subscriptions/vapid-public-key")
        .then((res) => setVapidPublicKey(res.data.configured ? res.data.public_key : null))
        .catch(() => setVapidPublicKey(null));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  /** Full Web Push enrollment flow for "this browser": ask OS permission,
   * register the service worker, subscribe with the server's VAPID key,
   * then hand the resulting endpoint/keys to the backend the same way any
   * other push device gets registered. Mirrors submitForm's ntfy/pushover
   * path but has no free-text target for the user to type -- the browser
   * supplies it. */
  const subscribeBrowser = async () => {
    setFormError(null);
    if (!browserPushSupported) {
      setFormError("This browser doesn't support push notifications.");
      return;
    }
    if (!vapidPublicKey) {
      setFormError("Browser push isn't configured on this server yet.");
      return;
    }
    setSubscribingBrowser(true);
    try {
      const permission = await Notification.requestPermission();
      if (permission !== "granted") {
        setFormError("Notification permission was denied -- allow notifications for this site and try again.");
        return;
      }
      ...
      const res = await api.post<PushSubscription>("/push-subscriptions", {
        label: form.label || "This browser",
        provider: "browser",
        include_non_critical: form.include_non_critical,
        endpoint: json.endpoint,
        p256dh: json.keys?.p256dh,
        auth: json.keys?.auth,
      });
      setSubscriptions((prev) => (prev ? [res.data, ...prev] : [res.data]));
      toast.success("This browser will now receive push notifications.");
      closeForm();
    } catch (err) {
      setFormError(extractErrorMessage(err, "Couldn't enable browser notifications."));
    } finally {
      setSubscribingBrowser(false);
    }
  };
  ...
  const submitForm = async (e: FormEvent) => {
    e.preventDefault();
    if (!editingId && form.provider === "browser") {
      // No target text to validate/submit -- the browser supplies its
      // own endpoint via the Push API, handled entirely by
      // subscribeBrowser().
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Get critical alerts and escalations pushed straight to your phone via ntfy or Pushover, or to this
            browser directly.
          </p>
        </div>
        <button
              ...
              <li key={sub.id} className="p-4 flex items-center gap-4">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate">
                    {sub.provider === "ntfy"
                      ? sub.target
                      : sub.provider === "browser"
                      ? "Web push subscription (this browser)"
                      : "•••• (Pushover key hidden)"}
                  </div>
            ...
            <form onSubmit={submitForm} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  {browserPushSupported && vapidPublicKey && !editingId && (
                    <button
                      type="button"
                      onClick={() => setForm((f) => ({ ...f, provider: "browser" }))}
                      className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium border transition-colors ${
              ...
              {form.provider === "browser" ? (
                <p className="text-xs text-slate-400 dark:text-slate-500">
                  Enables push notifications for this browser -- no topic or key to enter. You'll be asked to allow
